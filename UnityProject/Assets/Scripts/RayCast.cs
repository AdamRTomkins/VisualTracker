...
//      Record position of camera and direction and length 
//     
...
public class RayCast : MonoBehaviour {
    ...
    void Start () {
       ...
       viewportMarker.transform.localScale = new Vector3(viewportGridInterval, viewportGridInterval, viewportGridInterval);
        ...
        if (Time.time > nextActionTime)
        {
            ...
            if (hit.collider)
            {
                ...
                if (trackLocation)
                {
                    ...
                    {
                        // Calculate Voxel position in unrotated space
                        vectorFromCenter = gameObject.transform.position - viewport.transform.position;
